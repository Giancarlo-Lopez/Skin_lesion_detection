...
from my_package.class_segmentation_otsu import Class_Segmention_Otsu
from my_package.class_asymmetry import ClassAsymmetry
from my_package.class_border import ClassBorder
from my_package.class_color import ClassColor
from my_package.class_diameter import ClassDiameter
from my_package.class_for_operations_masks import ClassForOperationsMasks
import cv2
...
class GUI(tk.Frame):
    ...
    def load_image_file(self):
        ...
        if file_path:
            ...
            # Inicializar y usar la segmentación
            self.segmentation = Class_Segmention_Otsu(file_path)
            mask4, self.masked_image_4, resized_mask_4 = self.segmentation.otzu_edges_4()
            self.show_segmentation_graph()

            # Inicializar y usar las operaciones de máscaras
            self.operations_masks = ClassForOperationsMasks(mask4)

            self.touch_border = False
            self.height, self.width = self.operations_masks.mask.shape
            contours, _ = cv2.findContours(self.operations_masks.mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            ...
            if self.touch_border:
                mask_c = self.operations_masks.mask
            else:
                mask_c = self.operations_masks.center_mask()

            # Inicializar y usar asimetría
            self.asymmetry = ClassAsymmetry(mask_c)
            self.mask_right, self.mask_left, self.top_mask, self.down_mask = self.asymmetry.divide_images()
            self.union_left, self.asymmetry_left, self.union_top, self.asymmetry_top, self.percentage_asimetry_left, self.percentage_asimetry_down = self.asymmetry.asymmetry_method()
            self.perimeter_right, self.perimeter_left, self.perimeter_top, self.perimeter_down = self.asymmetry.calculate_perimeters()
            self.a1, self.a2, self.a = self.asymmetry.method_2()

            # Inicializar y usar el borde
            self.border = ClassBorder(mask4)
            self.B_score = self.border.index_borders_functions()

            # Inicializar y usar color
            self.color = ClassColor(file_path, resized_mask_4, False)
            self.Color_Score = self.color.color_score()

            # Inicializar y usar diámetro
            self.diameter = ClassDiameter(file_path, mask4)
    ...
    def display_results(self):
        self.tsd = round(self.a * 1.3 + self.B_score * 0.1 + self.Color_Score * 0.5 + self.diameter.score_diameter() * 0.5, 2)
        self.result = ''
        if self.tsd <= 4.75:
        ...
        results = [
            ...
            ('Puntaje Diámetro:', self.diameter.score_diameter()),
    ...
    def show_segmentation_graph(self):
        ...
        plt.subplot(1, 2, 1)
        plt.imshow(self.segmentation.image_color_rgb)
    ...
    def show_asymmetry_graph2(self):
        ...
        plt.imshow(self.asymmetry.rotated_image, cmap='gray')
        plt.axis('off')

        for start_point, end_point in self.asymmetry.segments_top:
            ax1.plot([start_point[0], end_point[0]], [start_point[1], end_point[1]], color='blue')

        for start_point, end_point in self.asymmetry.segments_down:
        ...
        for start_point, end_point in self.asymmetry.segments_left:
            ax2.plot([start_point[0], end_point[0]], [start_point[1], end_point[1]], color='red')

        for start_point, end_point in self.asymmetry.segments_right:
    ...
    def show_border_graph(self):
        ...
        for i in range(len(self.border.contour)):
            j = (i + 1) % len(self.border.contour)
            segment_index = int(self.border.angles[i] // 45)
            plt.plot([self.border.contour[i][0], self.border.contour[j][0]], [self.border.contour[i][1], self.border.contour[j][1]], color=colors(segment_index))

        plt.scatter(self.border.cx, self.border.cy, color='red', s=100, zorder=5, label='Centroide')
    ...
    def show_color_graph(self):
        ...
        plt.subplot(1, 2, 1)
        for color, count in self.color.significant_colors:
            plt.bar(color, count, color=np.array(self.color.reference_colors[color]) / 255, label=f"{color}: {count / self.color.total_pixels * 100:.2f}%")
        ...
        plt.subplot(1, 2, 2)
        result_image = Image.fromarray(self.color.classified_image)
    ...
    def show_diameter_graph2(self):
        ...
        plt.imshow(self.operations_masks.center_mask(), cmap='gray')
        plt.title("Diámetro", fontsize=10)
        plt.plot([self.diameter.pt1[0], self.diameter.pt2[0]], [self.diameter.pt1[1], self.diameter.pt2[1]], 'r-', linewidth=2)
        plt.scatter([self.diameter.pt1[0], self.diameter.pt2[0]], [self.diameter.pt1[1], self.diameter.pt2[1]], color='green', s=50)
        plt.axis('off')
